...
public interface DisplayRepository extends JpaRepositoryExportedFalse<Display, DisplayKey> {
    ...
    @RestResource(path = "userAndVideo", rel = "userAndVideo")
    @Query("select d from Display d where d.user.id = ?#{ principal?.id } and d.video.id = ?1")
    public Display findByUserIdAndVideoId(@Param("video_id") Long videoId);
